...
function ProcessingBar() {
  ...
  return (
    <div className="mt-2 h-0.5 overflow-hidden" style={{ backgroundColor: '#E5E5E0' }}>
      <motion.div
        className="h-full"
        style={{ width: `${width}%`, backgroundColor: '#CC0000' }}
...
function GridCard({ doc, isSelected, onToggle }) {
  ...
  return (
    <div
      className="overflow-hidden cursor-default transition-all"
    ...
    >
      ...
      >
        <span className="np-mono text-[11px] font-bold uppercase tracking-wide" style={{ color: '#7A7874' }}>
          {cfg.label}
        </span>
        ...
        <div className="absolute top-2.5 left-2.5">
          <input
            ...
            className={`w-4 h-4 cursor-pointer transition-opacity duration-150 ${isSelected ? 'opacity-100' : 'opacity-0'}`}
      ...
      <div className="px-4 py-3.5">
        <p className="np-sans text-[13px] font-semibold leading-snug line-clamp-2 mb-1.5" style={{ color: '#111111' }}>
          {doc.original_filename}
        </p>
        <p className="np-mono text-[11px]" style={{ color: '#AEABA6' }}>
      ...
      <AnimatePresence>
        {showFooter && (
          <motion.div
            ...
            style={{ borderTop: '1px solid #E5E5E0', backgroundColor: '#F5F0E8' }}
          >
            <div className="flex items-center px-4 py-2.5 gap-2">
              <button
                onClick={() => router.push('/dashboard')}
                className="flex-1 np-mono text-[10px] font-bold uppercase tracking-widest cursor-pointer hover:underline text-left transition-colors"
                style={{ color: '#CC0000' }}
              ...
              {confirming ? (
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={deleting}
                    className="np-mono text-[10px] font-bold uppercase tracking-widest cursor-pointer disabled:opacity-50"
                    style={{ color: '#DC2626' }}
                  >
                    {deleting ? '…' : 'Confirm'}
                  </button>
                  <button
                    ...
                    className="np-mono text-[10px] cursor-pointer"
                    style={{ color: '#AEABA6' }}
                  >
                    Cancel
                  </button>
                ...
                <button
                  onClick={handleDelete}
                  className="np-mono text-[10px] uppercase tracking-widest transition-colors cursor-pointer"
                  style={{ color: '#AEABA6' }}
...
function ListRow({ doc, isSelected, onToggle }) {
  ...
  return (
    ...
    >
      ...
      {/* Type label */}
      <span
        className="shrink-0 np-mono text-[9px] font-bold uppercase w-10 tracking-wide"
        style={{ color: '#AEABA6' }}
      >
        {cfg.label}
      </span>
      ...
      {/* Date */}
      <span className="w-20 shrink-0 np-mono text-[12px] hidden md:block" style={{ color: '#AEABA6' }}>
        {formatDate(doc.created_at)}
      </span>

      {/* Size */}
      <span className="w-16 shrink-0 np-mono text-[12px] hidden lg:block text-right" style={{ color: '#AEABA6' }}>
        ...
        <AnimatePresence>
          {menuOpen && (
            <motion.div
              ...
              className="absolute right-0 top-full mt-1 w-48 overflow-hidden z-10"
            ...
            >
              <button
                ...
                className="w-full flex items-center gap-3 px-4 py-2.5 np-sans text-[13px] cursor-pointer transition-colors text-left"
                style={{ color: '#111111' }}
                onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = '#F5F0E8' }}
              ...
              {confirming ? (
                <div className="flex items-center gap-3 px-4 py-2.5">
                  <button
                    onClick={handleDelete}
                    disabled={deleting}
                    className="np-mono text-[11px] font-bold uppercase tracking-widest disabled:opacity-50 cursor-pointer"
                    style={{ color: '#DC2626' }}
                  >
                  ...
                  <button
                    onClick={() => setConfirming(false)}
                    className="np-mono text-[11px] cursor-pointer"
                    style={{ color: '#AEABA6' }}
                  >
                    Cancel
                  </button>
